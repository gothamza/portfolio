...
\begin{minipage}[c]{0.80\textwidth}
    ...
    \cvtitle{Ingénieur en Intelligence Artificielle}\\[4pt]
    \cvmeta{%
...
Ingénieur IA, je conçois des systèmes LLM en production (RAG, agents, APIs) avec FastAPI, LangChain/LangGraph et Docker. Expérience en NLP, vision par ordinateur et déploiement full-stack.}
...
\begin{itemize}
    \item \role{Consultant en IA}{Avril 2025 -- Présent}
          \org{OBYSTECH SOLUTIONS, Rabat}
          Conseil IA en entreprise (ML, deep learning, NLP) et intégrations ERP (Odoo) ; plateforme d'agents IA multi-départements livrée en solo.
          \begin{itemize}
              \item Cas d'usage IA (CRM, ventes, inventaire, RH) ; prototypes NLP/ML branchés sur Odoo et processus métier.
              \item UI multi-chat Next.js (RAG, espaces d'équipe) et pipeline RAG FastAPI/LangGraph (embeddings HF, recherche hybride, MCP, mémoire PostgreSQL).
              \item Sécurisation des API LLM (chat, RAG, génération d'images) : schémas typés, JWT/OAuth2 et contrôle d'accès par rôles.
              \item Déploiement Docker Compose multi-services sur VPS (proxy inverse, SSL, zero-downtime).
    ...
    \item \role{Stagiaire en IA}{Février 2024 -- Août 2024}
          ...
          \begin{itemize}
              ...
              \item Endpoints Stable Diffusion~1.5, pipeline RAG Wikipédia/PDF, et authentification JWT sur FastAPI.
...
\begin{itemize}
    \item \textbf{Système Intelligent d'Évaluation} (2026) --- LLM/RAG examens, évaluation et feedback (BLEU, ROUGE, BERTScore) ; FastAPI, Streamlit, LangChain, Docker.
    \item \textbf{Computer Vision Playground} (2025) --- Streamlit/Docker : MTCNN, LBPH, YOLOv8, MediaPipe.
    \item \textbf{QA Darija} (2023) --- QA darija/anglais avec Transformer, Seq2Seq et LSTM.
    \item \textbf{Reconnaissance de Plaques} (2023) --- HOG + KNN/SVM ; \textbf{98,9\,\%} en test.
\end{itemize}
